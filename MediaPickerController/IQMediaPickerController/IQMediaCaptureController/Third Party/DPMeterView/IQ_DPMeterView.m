...
#define IQ_EPSILON     1e-6


@interface IQ_DPMeterView ()
...
- (void)commonInit
{
    ...
    self.meterType = IQ_DPMeterTypeLinearVertical;
...
{
    switch (_meterType) {
        case IQ_DPMeterTypeLinearVertical:
            [self setGradientOrientationAngle:M_PI_2];
            break;
            
        case IQ_DPMeterTypeLinearHorizontal:
            [self setGradientOrientationAngle:0];
            break;
            
        default:
            // enforce explicit declaration
            NSAssert(_meterType == IQ_DPMeterTypeLinearNone,
...
- (void)setMeterType:(IQ_DPMeterType)meterType
{
...
+ (NSArray *)intersectionPointsOfLineOrientedBy:(CGFloat)angle withBox:(CGRect)box
{
    ...
    if (fabs(angle - M_PI_2) < IQ_EPSILON) {
        // vertical "up"
    ...
    } else if (fabs(angle - 3*M_PI_2) < IQ_EPSILON) {
        // vertical "down"
    ...
    } else if (fabs(fabs(angle) - 2*M_PI_2) < IQ_EPSILON) {
        // horizontal "left"
...
- (NSArray *)gradientPoints:(CGFloat)angle
{
    ...
    NSArray *intersectionPoints = [IQ_DPMeterView intersectionPointsOfLineOrientedBy:angle withBox:box];
